...
public class SettingsFragment extends PreferenceFragmentCompat implements
    ...
    public void onCreatePreferences(Bundle bundle, String s) {
        ...
        Preference size = findPreference(getString(R.string.pref_size_key));
        if (size != null) size.setOnPreferenceChangeListener(this);
    }
    ...
    public boolean onPreferenceChange(Preference preference, Object newValue) {
        if (preference.getKey().equals(getString(R.string.pref_size_key))){
            try {
                ...
                if (0.0 < f && f <= 3.0) return true;
                Toast.makeText(getContext(),
                        "The value must be in the interval ]0,3]", Toast.LENGTH_LONG).show();
            } catch (NullPointerException e){
                Toast.makeText(
                    getContext(), "Please insert a value", Toast.LENGTH_LONG).show();
            } catch (NumberFormatException e){
                Toast.makeText(
                        getContext(), "Value must be decimal!", Toast.LENGTH_LONG).show();
            }
        }
        return false;
    }
