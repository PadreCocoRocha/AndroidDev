...
public class SettingsFragment extends PreferenceFragmentCompat
    ...
    // of Preference.
    public void setPreferenceSummary(Preference p, String key) {
        if (p instanceof ListPreference){
            ListPreference prefList = (ListPreference) p;
            int index = prefList.findIndexOfValue(key);
            if (index >= 0)
                prefList.setSummary(prefList.getEntries()[index]);
        }
    }
