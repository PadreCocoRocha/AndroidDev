...
//        TestUtil.insertFakeData(mDb);
    ...
    public void addToWaitlist(View view) {
        ...
        String newName = mNewGuestNameEditText.getText().toString();
        String newSize = mNewPartySizeEditText.getText().toString();

        if (newName.length() == 0 || newSize.length() == 0) return;
        ...
        try {
            newPartySize = Integer.parseInt(newSize);
        } catch (Exception e){
            Log.w(LOG_TAG, "Shit happened during conversion!: " + e.getMessage());
            return;
        }
        // COMPLETE (14) call addNewGuest with the guest name and party size
        addGuest(newName, newPartySize);
    ...
    public void addGuest(String name, int size){
